              ...
              <form name="frmDatos">
                <div class="card-body">
                  ...
                  <div class="row">
                    ...
                    <input type="hidden" name="txtIdCompra" id="txtIdCompra" value="<?php echo $compra->getIdCompra(); ?>">
                          ...
                          <select name="cboProveedor" class="form-control" id="cboProveedor">
                            <?php foreach ($listadoProveedor as $proveedor):
                              ...
                              $selected = '';
                                ...
                                if ($compra->proveedor->getIdProveedor() == $proveedor->getIdProveedor()) {
                      ...
                      <div class="form-group">
                        <label for="cboEstado">Estado:</label>
                          <select name="cboEstado" class="form-control" id="cboEstado">
                            ...
                            <?php foreach ($listadoEstado as $estadoCompra):
                                $selected = '';
                                
                                if ($compra->estadoCompra->getIdEstadoCompra() == $estadoCompra->getIdEstadoCompra()) {
                        ...
                        <label for="cboTipoPago">Tipo de pago:</label>
                          <select name="cboTipoPago" class="form-control" id="cboTipoPago">
                              ...
                              <?php foreach ($listadoTipoPago as $tipoPago):
                                ...
                                if ($compra->getIdTipoPago() == $tipoPago->getIdTipoPago()) {
...
$.ajax({
  type: 'GET',
    url : '../../utils/productos/obtenerCompraPorId.php',
    data: {'idCompra': $('#txtIdCompra').val() },
    success: function(data){
        var datosDetalleCompra = JSON.parse(data);
        //console.log(datosDetallePedido);
        for (var x=0; x < datosDetalleCompra._arrDetalleCompra.length; x++){
          console.log(datosDetalleCompra._arrDetalleCompra[x]);

            let subtotal = calcularSubtotal(datosDetalleCompra._arrDetalleCompra[x]._cantidad, datosDetalleCompra._arrDetalleCompra[x]._precio);

           let items = {}; //items del detalle


        items['id_producto'] = datosDetalleCompra._arrDetalleCompra[x]._idProducto;
        items['cantidad'] = datosDetalleCompra._arrDetalleCompra[x]._cantidad;
        items['precio'] = datosDetalleCompra._arrDetalleCompra[x]._precio;

            detalle_compra.push(items); //armando detalle para el envio

        $('#detalleCompra tr:last').after('<tr id=' + indice +'><td>' + datosDetalleCompra._arrDetalleCompra[x]._idProducto + '</td><td>' + datosDetalleCompra._arrDetalleCompra[x].producto._descripcion + '</td><td>' + datosDetalleCompra._arrDetalleCompra[x]._cantidad + '</td><td>$' + datosDetalleCompra._arrDetalleCompra[x]._precio + '</td><td>$' + subtotal + '</td><td> <button type="button" onclick="eliminarArticulo(' + indice + ')" class="btn btn-danger"><i class="far fa-trash-alt"></i></button></td></tr>')
